...
const QuestionItemEditor = memo(function QuestionItemEditor({
    ...
    const handleSaveEdits = async () => {
        // Physics logic: using same endpoint as verify but maybe without verified=true?
        // Actually, let's keep it simple and just use the verify endpoint for Physics too if needed,
        // or just let it be verified directly.
        // The backend for Physics doesn't have a separate /edit/ endpoint in the snippet.
        // It has /book/{book_id}/{page_name}/verify-item/{question_index}
        // So saving edits should probably just be handled in UI until verification.
        setIsEditing(false);
    };

    const verifyQuestion = async () => {
        try {
            // Physics consolidated API
            const url = `${API_BASE_URL}/api/v1/physics/book/${bookName}/${pageName}/verify-item/${index}`;
        ...
        <main className="dashboard-container">
            <header className="fixed-header" style={{ zIndex: 10 }}>
                <div className="header-content" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                        <h1>IIT JEE Physics Pipeline 🚀</h1>
                        <p style={{ color: 'var(--text-light)', opacity: 0.8, marginTop: '4px' }}>
                            Physics Extraction logic and 3-LLM Agentic Workflow
                        </p>
                    </div>
...
function ExtractionPipeline() {
    ...
    useEffect(() => {
        const fetchTaxonomy = async () => {
            try {
                // Physics specific taxonomy
                const res = await fetch(`${API_BASE_URL}/api/v1/physics/taxonomy-data/list`);
    ...
    useEffect(() => {
        ...
        const checkResult = async () => {
            try {
                // Physics specific result check
                const res = await fetch(`${API_BASE_URL}/api/v1/physics/book/${bookId}/${currentPage.name}/result`);
                if (res.ok) {
                    const data = await res.json();
                    if (data.type === 'multi' || data.questions || data.type === 'single') {
                        setResult(data);
                        setStatus('completed');
    ...
    const fetchBookPages = useCallback(async (bid: string) => {
        if (!bid) return;
        try {
            // Physics specific pages list
            const res = await fetch(`${API_BASE_URL}/api/v1/physics/book/${bid}/pages`);
            const data = await res.json();
            if (data && data.pages) {
                setBookPages(prev => {
    ...
    const handleUpload = async () => {
        ...
        setStatus('uploading');
        setMessage('Uploading Physics Book PDF...');
        setError(null);
        setResult(null);
        setVerifiedIndexes([]);
        ...
        try {
            const res = await fetch(`${API_BASE_URL}/api/v1/physics/book/upload`, {
                method: 'POST',
                body: formData,
            });
            const data = await res.json();
    ...
    const handleMarkPage = async (pageName: string, action: 'skip' | 'process') => {
        if (!bookId) return;
        try {
            await fetch(`${API_BASE_URL}/api/v1/physics/book/${bookId}/mark`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    const startPolling = (jobId: string) => {
        ...
        const poll = async () => {
            try {
                // Physics specific status endpoint
                const res = await fetch(`${API_BASE_URL}/api/v1/physics/extraction-status/${jobId}`);
    ...
    const handleProcessPage = async (pageName: string) => {
        ...
        setStatus('uploading');
        setMessage(`Processing Physics Page: ${pageName}...`);
        setResult(null);
        setVerifiedIndexes([]);

        try {
            const response = await fetch(`${API_BASE_URL}/api/v1/physics/book/${bookId}/${pageName}/process`, {
                method: 'POST'
            });
        ...
        <div style={{ height: '100%' }}>
            {bookId ? (
                <div className="split-view">
                    <div className="panel" style={{ height: 'calc(100vh - 120px)', overflowY: 'auto' }}>
                        <div className="book-header">
                            <h2 style={{ fontSize: '18px', fontWeight: '600' }}>Physics Book Viewer</h2>
                            ...
                            {!currentPage ? (
                                ...
                                <>
                                    ...
                                    <div className="page-actions-overlay" style={{ marginTop: '0', width: '100%', gridTemplateColumns: '1fr 1fr' }}>
                                        ...
                                        >
                                            {result ? 'Reprocess Page AI' : 'Process Physics AI'}
                                        </button>
                                    </div>
                                </>
                            )}
                    ...
                    <div className="panel result-panel-container" style={{ height: 'calc(100vh - 120px)', overflowY: 'auto' }}>
                        ...
                        {result && (
                            <div className="result-panel">
                                ...
                                <p style={{ color: 'var(--text-light)', marginBottom: '20px', marginTop: '20px' }}>
                                    Review, edit, and individually verify questions for "{currentPage?.name}".
                                </p>
                        ...
                        {!result && status === 'idle' && (
                            <div className="empty-state">
                                <span>⚡</span>
                                <p>Select a physics page and click Process AI.</p>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div style={{ maxWidth: '600px', margin: '0 auto', padding: '40px' }} className="control-panel">
                    <h2>Upload Physics Book</h2>
                    <p style={{ color: 'var(--text-light)', marginTop: '8px' }}>
                        Upload a Physics PDF book to begin extraction.
                    </p>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', marginTop: '24px' }}>
                        ...
                        >
                            {status === 'uploading' ? 'Uploading...' : 'Upload Physics PDF'}
                        </button>
